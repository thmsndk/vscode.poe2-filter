...
1. Install **Path of Exile 2 Filter** from the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=thmsn.poe2-filter) (or search for it in the Extensions view).
...
`pnpm package` runs `vsce package --no-dependencies` and produces a `.vsix` you can install locally via the Extensions view ("Install from VSIX...").
...
2. Authenticate once with `pnpm exec vsce login thmsn`.
3. Build and publish:
