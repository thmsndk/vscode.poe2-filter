...
export class Lexer {
  ...
  private readCommentOrHeader(): Token {
    ...
    // Check if it is a commented block
    ...
    // Check all commented tokens using secondWord
    if (secondWord === "Show" || secondWord === "Hide") {
      const end =
        this.position + firstWord.length + 1 /* space */ + secondWord.length;
    ...
    if (this.isCondition(secondWord)) {
      // count indentations before #
      const indentations = (line.match(/^\s+/) || [""])[0].length;
      const indentationsBetweenFirstAndSecondWord = (line.match(
        `${firstWord}(\\s+)${secondWord}` // Match exactly # and BaseType with whitespace between
      ) || [""])[1].length;
      ...
      const end =
        ...
        indentations +
        firstWord.length +
        indentationsBetweenFirstAndSecondWord +
        secondWord.length;
      this.advanceToPosition(end);
